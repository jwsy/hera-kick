...
The four Zeus disguises use the actual pixel art from `sprites-init.png`, extracted into `assets/sprites/enemies-sheet.png` (frame boxes in the `ENEMY_FRAMES` constant). In the reference sheet each enemy is partly covered by the kick-impact spark and Hera's leg; the extraction (occlusion polygons fitted to the *measured* spark extents, color-based residue sweep with per-enemy face/beak/horn protection boxes, clone-stamp/patch texture repair) removes those. Downscaling is box-average + 40-color quantize, which preserves thin art features (brows, nostrils, eyespots) that plain nearest-neighbor sampling randomly dropped. Features that the art hides behind the spark entirely (the bull's nostril and mouth) are drawn in by the build script at full res before downscaling. Both sheets are regenerated by `tools/build_enemies.py` and `tools/build_hera.py` (run from the repo root; they print the frame metadata for the `*_FRAMES` constants in `app.js`). Sprites are pre-scaled to game size and blitted 1:1 with `imageSmoothingEnabled = false` at integer coordinates to keep the 16-bit look crisp. Each enemy has two baked frames (base + squash variant) alternated at `ENEMY_ANIM_HZ`, driven by the module-level `animT` clock (accumulated in `update()`, so animation freezes on pause) plus a per-enemy random `phase` set at spawn; flying enemies bob, the bull bounces with its gallop, and the cloud gets extra procedural lightning crackle (`drawBolt`) on top of the bolts baked into its sprite. The collectible peacock lives in the same sheet (`peacock0`/`peacock1`, cut clean from the art's bottom row) and is drawn the same crisp way with a two-frame strut plus a gold pickup glint.
