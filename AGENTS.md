...
`assets/sprites/hera-sheet.png` — a single row of 7 variable-width frames cut from the reference art `sprites-init.png` ("Hera, Queen of the Gods"): `idle0`, `idle1`, and a 4-frame run cycle `walk0` → `pass0` → `walk1` → `pass1`, plus `kick` (impact spark baked in). The pass frames are generated from the walk frames by squashing the hem/feet band (legs tucked, airborne). The sheet ships with a transparent background — there is **no** load-time pixel cleanup. Frame boxes and anchors (`ax`) live in the `HERA_FRAMES` constant in `app.js`. Run-cycle anchors sit on the **head/torso centroid** (not the feet) so her body stays steady between frames — foot anchoring caused visible twitching. Run frames are also top-aligned to `RUN_BASE_H` so the shorter pass frames lift her feet rather than drop her head; footfalls and landings kick up dust puffs. Jumping reuses `pass1` with a velocity-based tilt. Regenerating: crop the top-row poses from `sprites-init.png`, key out the purple background, derive the pass frames, and update `HERA_FRAMES`.

### Enemies

The four Zeus disguises use the actual pixel art from `sprites-init.png`, extracted into `assets/sprites/enemies-sheet.png` (frame boxes in the `ENEMY_FRAMES` constant). In the reference sheet each enemy is partly covered by the kick-impact spark and Hera's leg; the extraction (occlusion polygons fitted to the *measured* spark extents, color-based residue sweep with per-enemy face/beak/horn protection boxes, clone-stamp/patch texture repair) removes those. Downscaling is box-average + 40-color quantize, which preserves thin art features (brows, nostrils, eyespots) that plain nearest-neighbor sampling randomly dropped. Sprites are pre-scaled to game size and blitted 1:1 with `imageSmoothingEnabled = false` at integer coordinates to keep the 16-bit look crisp. Each enemy has two baked frames (base + squash variant) alternated at `ENEMY_ANIM_HZ`, driven by the module-level `animT` clock (accumulated in `update()`, so animation freezes on pause) plus a per-enemy random `phase` set at spawn; flying enemies bob, the bull bounces with its gallop, and the cloud gets extra procedural lightning crackle (`drawBolt`) on top of the bolts baked into its sprite. The collectible peacock lives in the same sheet (`peacock0`/`peacock1`, cut clean from the art's bottom row) and is drawn the same crisp way with a two-frame strut plus a gold pickup glint.
